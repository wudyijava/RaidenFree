...
void BmobObject::increment(string column,int value /*= 1*/){
   // this->addParams(column,__Integer::create(value));
}

void BmobObject::setValue(string key,cocos2d::Ref *object){
    this->enParamsToHttp(key,object);
}

void BmobObject::setValue(string key,__Array* array){
    ...
    dict->setObject(__String::createWithFormat("%s",op.c_str()),"__op");
...
void BmobObject::remove(string name){
    ...
    __String* pValue2 = __String::create("Delete");
...
void BmobObject::add(string column,Ref* object){
...
void BmobObject::add(string column,__Array* array){
...
void BmobObject::enParamsToHttp(string key,cocos2d::Ref *object){
    this->m_mapData.insert(pair<string, Ref *>(key, object));
}

Ref* BmobObject::getParams(string key){
    std::map<string, Ref *>::iterator it = m_mapData.find(key);
    if (it!=m_mapData.end()) {
        return it->second;
    }
...
void BmobObject::onHttpRequestCompleted(cocos2d::Node *pSender,void *data){
                ...
                switch(_opType){
                    case HTTP_OP_Type::_bHTTP_SAVE:{
                        if (strcmp(m_tableName.c_str(),BmobSDKInit::USER_TABLE.c_str()) == 0)
                        {
                            ...
                            UserDefault::getInstance()->setStringForKey("user_id",objectId);
                            UserDefault::getInstance()->setStringForKey("user_session",session);
